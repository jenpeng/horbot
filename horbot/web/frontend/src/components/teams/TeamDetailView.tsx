      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-2 text-xs text-surface-500">
            {t('teams.teamDetail.workspaceLabel')}{selectedTeam.effective_workspace || selectedTeam.workspace || t('teams.teamDetail.defaultTeamDirectory')}
          </p>
        </div>
        <button
          onClick={onEditTeam}
        ...
        >
          ...
          {t('teams.teamDetail.editTeam')}
        </button>
      </div>
      <div className="mt-4 flex flex-wrap gap-2 text-xs text-surface-500">
        <span className={getBadgeClassName('neutral')}>{t('teams.teamDetail.memberCount', { count: selectedTeamAgents.length })}</span>
        <span className={getBadgeClassName(selectedTeamLead ? 'primary' : 'neutral')}>
          {selectedTeamLead ? t('teams.teamDetail.leadAssigned', { name: selectedTeamLead.name }) : t('teams.teamDetail.noLeadAssigned')}
        </span>
        <span className={getBadgeClassName('neutral')}>{t('teams.teamDetail.capabilityCount', { count: selectedTeamCapabilitiesCount })}</span>
        <span className={getBadgeClassName('neutral')}>{selectedTeam.workspace || selectedTeam.effective_workspace ? t('teams.summary.customWorkspace') : t('teams.summary.defaultWorkspace')}</span>
      </div>
    </div>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <div className="lg:col-span-2">
        <div className="flex items-center justify-between gap-3">
          <div>
            <h3 className="text-lg font-semibold text-surface-900">{t('teams.teamDetail.dailyCollabTitle')}</h3>
            <p className="mt-1 text-sm text-surface-500">{t('teams.teamDetail.dailyCollabSubtitle')}</p>
          </div>
        </div>
      </div>
      <div className="bg-white rounded-2xl border border-surface-200 p-6 transition-shadow" data-focus-anchor="team-members">
        <h3 className="text-lg font-semibold text-surface-900 mb-4">{t('teams.teamDetail.membersTitle')}</h3>
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-surface-500">{agent.description || t('teams.agentOverview.noDescription')}</p>
                  {profile.responsibility && (
                    <p className="mt-1 text-xs text-surface-500">{t('teams.teamDetail.responsibilityLabel', { value: profile.responsibility })}</p>
                  )}
                  <p className="mt-1 text-[11px] text-surface-400">{t('teams.teamDetail.priorityLabel', { value: priorityLabel })}</p>
                </div>
                <div className="flex items-center gap-2">
      ...
      <div className="lg:col-span-2 bg-white rounded-2xl border border-surface-200 p-6 transition-shadow" data-focus-anchor="team-collaboration">
        <div className="mb-4">
          <h3 className="text-lg font-semibold text-surface-900">{t('teams.teamDetail.collabFlowTitle')}</h3>
          <p className="mt-1 text-sm text-surface-500">{t('teams.teamDetail.collabFlowSubtitle')}</p>
        </div>
      ...
      <div className="lg:col-span-2 pt-2">
        <div className="flex items-center justify-between gap-3">
          <div>
            <h3 className="text-lg font-semibold text-surface-900">{t('teams.teamDetail.assetsTitle')}</h3>
            <p className="mt-1 text-sm text-surface-500">{t('teams.teamDetail.assetsSubtitle')}</p>
          </div>
        </div>
      </div>

      <div className="bg-white rounded-2xl border border-surface-200 p-6">
        <h3 className="text-lg font-semibold text-surface-900 mb-4">{t('teams.teamDetail.capabilityDistributionTitle')}</h3>
        <div className="space-y-3">
            ...
            return (
              <div key={capability} className="p-3 rounded-xl bg-surface-50">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-xs text-surface-500">{t('teams.teamDetail.agentCount', { count: agentsWithCapability.length })}</span>
                </div>
          ...
          {selectedTeamAgents.flatMap((agent) => agent.capabilities).length === 0 && (
            <div className="text-center py-8 text-surface-500">
              {t('teams.teamDetail.noCapabilities')}
            </div>
          )}
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-surface-200 p-6 lg:col-span-2 transition-shadow" data-focus-anchor="team-workspace">
        <h3 className="text-lg font-semibold text-surface-900 mb-4">{t('teams.teamDetail.workspaceTitle')}</h3>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="p-4 rounded-xl bg-surface-50 text-center">
            ...
            <p className="text-sm font-medium text-surface-900">{t('teams.teamDetail.workspaceCardShared')}</p>
            <p className="text-xs text-surface-500 mt-1">{t('teams.teamDetail.workspaceCardSharedDesc')}</p>
          </div>
          <div className="p-4 rounded-xl bg-surface-50 text-center">
            ...
            <p className="text-sm font-medium text-surface-900">{t('teams.teamDetail.workspaceCardMemory')}</p>
            <p className="text-xs text-surface-500 mt-1">{t('teams.teamDetail.workspaceCardMemoryDesc')}</p>
          </div>
          <div className="p-4 rounded-xl bg-surface-50 text-center">
            ...
            <p className="text-sm font-medium text-surface-900">{t('teams.teamDetail.workspaceCardBus')}</p>
            <p className="text-xs text-surface-500 mt-1">{t('teams.teamDetail.workspaceCardBusDesc')}</p>
          </div>
          <div className="p-4 rounded-xl bg-surface-50 text-center">
            ...
            <p className="text-sm font-medium text-surface-900">{t('teams.teamDetail.workspaceCardTasks')}</p>
            <p className="text-xs text-surface-500 mt-1">{t('teams.teamDetail.workspaceCardTasksDesc')}</p>
          </div>
        </div>
      </div>
    </div>
