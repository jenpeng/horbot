...
const StatusOverviewPanel = ({
  ...
  <div className="flex flex-col gap-10" role="tabpanel" id="overview-panel" aria-labelledby="overview-tab">
    <div className="rounded-xl border border-accent-orange/30 bg-accent-orange/10 px-4 py-3 text-sm text-surface-800">
      <p className="font-semibold text-surface-900">{t('status.overview.accessBoundaryTitle')}</p>
      <p className="mt-1">
        {t('status.overview.accessBoundaryBody')}
      </p>
    </div>
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      <StatusMetricCard
        ...
        value={status.status === 'running' ? t('system.running') : status.status}
        icon={(
      ...
      />
      <StatusMetricCard
        label={t('dashboard.systemInfo.version')}
        value={status.version || notAvailable}
        icon={(
          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
      ...
      <StatusMetricCard
        ...
        value={status.uptime || notAvailable}
        icon={(
          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
    ...
    {memoryStats?.details?.metrics && (
      <Card padding="lg" className="group relative overflow-hidden transition-all duration-300 hover:shadow-lg hover:shadow-accent-emerald/10">
        ...
        <CardHeader title={t('status.overview.memoryRecallTitle')} className="relative z-10" />
        <CardContent className="relative z-10 space-y-5">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <StatusMetricCard
              label={t('status.overview.recallAvg')}
              value={formatDurationMs(memoryStats.details.metrics.recall.avg_latency_ms)}
              subtext={t('status.overview.recallCount', { count: memoryStats.details.metrics.recall.count })}
              status={memoryStats.details.metrics.recall.avg_latency_ms > 800 ? 'warning' : 'success'}
            />
            <StatusMetricCard
              label={t('status.overview.recallCandidates')}
              value={memoryStats.details.metrics.recall.avg_candidates_count.toFixed(1)}
              subtext={t('status.overview.averageCandidates')}
              status="info"
            />
            <StatusMetricCard
              label={t('status.overview.consolidationAvg')}
              value={formatDurationMs(memoryStats.details.metrics.consolidation.avg_latency_ms)}
              subtext={t('status.overview.consolidationSummary', {
                success: memoryStats.details.metrics.consolidation.success_count,
                failure: memoryStats.details.metrics.consolidation.failure_count,
              })}
              status={memoryStats.details.metrics.consolidation.failure_count > 0 ? 'warning' : 'success'}
            />
            <StatusMetricCard
              label={t('status.overview.memorySize')}
              value={`${memoryStats.total_size_kb.toFixed(1)} KB`}
              subtext={t('status.overview.currentCoreFiles', { count: memoryStats.details.metrics.growth.current_entries })}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
              <div className="flex items-center justify-between gap-3">
                <p className="text-sm font-semibold text-surface-900">{t('status.overview.lastRecallTitle')}</p>
                <span className="text-xs text-surface-500">
                  {memoryStats.details.metrics.recall.last_samples.at(-1)?.timestamp
                    ? new Date(memoryStats.details.metrics.recall.last_samples.at(-1)!.timestamp).toLocaleString(intlLocale)
                    : t('status.overview.none')}
                </span>
              </div>
              <div className="mt-3 space-y-2 text-xs text-surface-600">
                <div>{t('status.overview.lastSelectedMemories', { count: memoryStats.details.metrics.recall.last_selected_memory_ids.length })}</div>
            ...
            <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
              <div className="flex items-center justify-between gap-3">
                <p className="text-sm font-semibold text-surface-900">{t('status.overview.memoryGrowthTitle')}</p>
                <span className="text-xs text-surface-500">{t('status.overview.recentChange')}</span>
              </div>
              <div className="mt-3 grid grid-cols-2 gap-3 text-sm">
                <div className="rounded-2xl bg-white px-3 py-3 ring-1 ring-slate-200">
                  <div className="text-xs text-surface-500">{t('status.overview.fileDelta')}</div>
                ...
                <div className="rounded-2xl bg-white px-3 py-3 ring-1 ring-slate-200">
                  <div className="text-xs text-surface-500">{t('status.overview.sizeDelta')}</div>
                  <div className="mt-1 font-semibold text-surface-900">
    ...
    <Card padding="lg" className="group relative overflow-hidden transition-all duration-300 hover:shadow-lg hover:shadow-primary-500/10">
      ...
      <CardHeader title={t('status.overview.performanceTitle')} className="relative z-10" />
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      <StatusMetricCard
        label={t('status.overview.cronService')}
        value={status.services.cron.enabled ? t('system.running') : t('system.stopped')}
        subtext={t('status.overview.jobsScheduled', { count: status.services.cron.jobs_count })}
        icon={(
          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
      ...
      />
      <StatusMetricCard
        label={t('status.overview.aiAgent')}
        value={status.services.agent.initialized ? t('status.overview.ready') : t('status.overview.notReady')}
        subtext={t('status.overview.modelValue', { value: status.config.model || notAvailable })}
        icon={(
          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
