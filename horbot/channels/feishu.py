...
from horbot.config.schema import FeishuConfig

# Global flag to track if SSL patch has been applied
_SSL_PATCH_APPLIED = False


def _apply_ssl_patch() -> None:
    """
    Apply SSL verification bypass for environments with proxy/firewall.
    
    This is needed when:
    - Corporate proxy intercepts HTTPS traffic with self-signed certificates
    - Network firewall performs SSL inspection
    - Development environment with custom CA
    
    The patch:
    1. Sets Python's default HTTPS context to skip verification
    2. Monkey patches websockets.connect to pass unverified SSL context
    ...
    # Disable SSL verification for Python's HTTPS context
    ssl._create_default_https_context = ssl._create_unverified_context
    
    # Monkey patch websockets to disable SSL verification
    try:
        import websockets
        ...
        logger.info("SSL verification bypass applied for Feishu WebSocket connection")
    except ImportError:
        pass
...
class FeishuChannel(BaseChannel):
    ...
    def __init__(self, config: FeishuConfig, bus: MessageBus, **channel_kwargs):
        super().__init__(config, bus, **channel_kwargs)
        self.config: FeishuConfig = config
        self._client: Any = None
        self._ws_client: Any = None
        self._ws_thread: threading.Thread | None = None
        self._processed_message_ids: OrderedDict[str, None] = OrderedDict()  # Ordered dedup cache by message_id
        self._recent_contents: OrderedDict[str, float] = OrderedDict()  # Content hash -> timestamp for content dedup
    ...
    async def start(self) -> None:
        ...
        # Apply SSL verification bypass if configured
    ...
    async def _download_and_save_media(
        ...
        """
        loop = asyncio.get_running_loop()
        from horbot.utils.paths import get_data_dir
    ...
    async def _on_message(self, data: "P2ImMessageReceiveV1") -> None:
        ...
        try:
            ...
            # Deduplication check by message_id
            message_id = message.message_id
            if message_id in self._processed_message_ids:
                return
            self._processed_message_ids[message_id] = None

            # Trim message_id cache
            ...
            # This prevents processing duplicate messages from Feishu reconnection
            import time
            import hashlib
            content_hash = hashlib.md5(f"{sender_id}:{message.content}".encode()).hexdigest()
            current_time = time.time()
            if content_hash in self._recent_contents:
                last_time = self._recent_contents[content_hash]
                if current_time - last_time < 300:  # 5 minutes
                    logger.debug("Skipping duplicate content from {} ({}s ago)", sender_id, int(current_time - last_time))
                    return
            self._recent_contents[content_hash] = current_time
            # Trim content cache (keep last 100 entries, expire after 5 minutes)
