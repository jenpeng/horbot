...
- a compact task workbench button in the chat header that opens a popover summarizing the latest request, files, execution steps, tools, relay count, and current stage from already loaded message metadata
...
The task workbench is UI-derived and does not ask the model to generate extra status text. It reads existing messages, files, execution steps, tool names, and relay metadata, so it improves scanability without increasing prompt tokens. The chat header only shows a compact `Task Workbench` button; click it when you need the popover, so the message list is not covered by a persistent card. Quick actions in the workbench only prefill the input box, so no model request is made until you explicitly send. These quick actions are state-aware: failures, attachments, long context, and team relay conversations can surface different suggested next steps. `Copy summary` copies the current workbench state for reuse in a new chat or handoff. `Use summary` fills that same summary into the message input for explicit continuation without sending automatically. `Search request` opens history search with the latest request as the query, reusing loaded-history and full-history search without adding model calls.
