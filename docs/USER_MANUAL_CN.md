...
预览页默认只加载当前页和相邻页，不会一次性生成几百上千个页面 DOM。关闭或卸载预览时，前端会通知后端删除中间 PDF；已经渲染出的 PNG 页缓存可以短期保留用于再次打开加速，但后端会自动清理 3 天未使用的页图片缓存。
...
- 聊天头部会显示紧凑的 `任务工作台` 按钮，点击后弹出面板，从已加载消息、附件、执行步骤、工具名和接力状态里自动汇总最近请求、当前阶段、文件数和执行统计

`任务工作台` 不会让模型额外生成状态文本，也不会把这些 UI 信息回灌到 prompt。它只使用本地已有消息元数据，因此提升可读性但不会增加默认 token 消耗。聊天头部只常驻一个紧凑按钮，需要时点击打开弹出面板，不会用大卡片遮挡消息列表。工作台里的快捷操作只会预填输入框，用户显式发送前不会产生模型请求。这些快捷操作会根据状态动态出现：失败、附件、长上下文、团队接力会触发不同的下一步建议。`复制摘要` 可以把当前工作台状态复制出来，用于新会话或转交。`填入摘要` 会把同一份摘要带入输入框，方便用户显式确认后继续推进，不会自动发送。`搜索请求` 会用最近请求打开历史搜索，复用已加载历史和服务端全量历史检索，不增加模型调用。
...
- 在 `图谱` 标签页查看当前 Agent 的技能、`references/` 引用文件和相关技能关系
...
Skills 页面的 `图谱` 标签会基于当前 Agent 的技能目录生成关系图。

图谱当前包含：

- 系统技能和自定义技能节点
- `references/` 下的引用资料节点
- `has_reference`、`similar_to`、`related_to` 等关系
- 节点数、关系数、引用数等摘要
- 焦点式一跳关系图，先选中一个技能，再查看它直接关联的 references 和相邻技能，避免大量节点堆在一起

当用户通过 Web UI 创建、编辑、导入、删除、启用/禁用技能，整理自动技能，转为系统技能，或 Agent 自动沉淀技能时，Horbot 会自动刷新当前 Agent 的持久化图谱。如果直接在文件系统里手动改了技能文件，可以点击 `重建图谱` 立即重新生成。

图谱文件位于当前 Agent workspace 的 `.horbot-agent/skill_graph.json`。Agent 运行时会把图谱作为技能摘要里的轻量提示使用，只提示相关技能和 references 文件位置，不会在每次请求里直接塞入完整资料内容。
