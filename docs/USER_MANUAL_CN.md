...
- 会话概览区会显示 `任务工作台`，从已加载消息、附件、执行步骤、工具名和接力状态里自动汇总最近请求、当前阶段、文件数和执行统计

`任务工作台` 不会让模型额外生成状态文本，也不会把这些 UI 信息回灌到 prompt。它只使用本地已有消息元数据，因此提升可读性但不会增加默认 token 消耗。默认保持紧凑展示，需要查看工具和执行步骤拆解时再点击 `详情` 展开。工作台里的快捷操作只会预填输入框，用户显式发送前不会产生模型请求。这些快捷操作会根据状态动态出现：失败、附件、长上下文、团队接力会触发不同的下一步建议。`复制摘要` 可以把当前工作台状态复制出来，用于新会话或转交。`填入摘要` 会把同一份摘要带入输入框，方便用户显式确认后继续推进，不会自动发送。
