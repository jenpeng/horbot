...
`任务工作台` 不会让模型额外生成状态文本，也不会把这些 UI 信息回灌到 prompt。它只使用本地已有消息元数据，因此提升可读性但不会增加默认 token 消耗。默认保持紧凑展示，需要查看工具和执行步骤拆解时再点击 `详情` 展开。工作台里的快捷操作只会预填输入框，用户显式发送前不会产生模型请求。这些快捷操作会根据状态动态出现：失败、附件、长上下文、团队接力会触发不同的下一步建议。
