...
- **2026-04-15** 🌐 Agent 联网能力调整为默认优先使用原生 `web_access`，并将 `web-access` 代理内置到项目中；`./horbot.sh start` 会一并启动代理并自动尝试拉起带 remote debugging 的 Chrome。
...
当前 `./horbot.sh start` 也会同时拉起项目内置的 `web-access` 代理服务，默认监听 `127.0.0.1:3456`，并自动尝试启动带 remote debugging 的 Chrome，无需再单独 clone 外部仓库或手工起代理。

当前这轮聊天与附件能力落地没有新增第三方依赖，因此不需要额外修改 `package.json`、`pyproject.toml` 或部署 `yml`。

当前聊天界面还做了两类交互优化：

- Assistant 消息块与 Markdown 默认间距更紧凑，减少大面积留白
- 团队接力中的等待态会直接显示“谁把任务交给谁”以及当前是继续讨论还是回到最终总结
- 当单聊中的 Agent 把任务派发到团队接力时，Web Chat 会自动切到团队会话；最终镜像汇报返回后，再自动切回原单聊，并在顶部显示短暂的接力导航提示
- 当请求失败或前端检测到流式异常时，顶部黄条和 turn 头部会直接显示 `request_id`，便于排查 provider、网关和会话日志
- 如果某次前端超时后，后端其实已经把成功回复落盘，聊天页会按 `request_id` 自动回查历史并收敛假超时提示
- 对于超长 Web 单聊上下文，后端会先按 token 预算自动压缩旧上下文；如果模型因 `finish_reason=length` 被截断，还会自动续答并把后续内容拼接成一条完整回复继续输出
...
- 复杂任务会自动触发规划检测
- 如果需要规划，前端会直接展示计划和确认步骤
- `agents.defaults.models.planning` 仍然有效，但它表示内部规划场景使用的模型，而不是一个需要手动输入的命令

## 🤖 创建 Agent

当前“团队管理 / 多 Agent 管理”中的创建 Agent 弹窗已经支持直接填写：
